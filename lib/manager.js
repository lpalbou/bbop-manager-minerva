...
var manager = function(barista_location, namespace, user_token, engine, mode){
	...
	args['requests'] = str;
	// // NOTE/WARNING: While the node engine seems fine with taking
	// // objects and arrays when making requests without serializing
	// // them, it seems that jQuery is not so forgiving, meaning
	// // that lower-level bits need to be re-encoded. In our case,
	// // since groups (provided-by) is an array, we need to encode
	// // it to make sure is survives the trip in the browser.
	// // Needless to say, this was causing some really weird bugs.
	// // Would a jQuery upgrade help here?
	// if( args['provided-by'] ){
	//     var prv = args['provided-by'];
	//     var prv_str = JSON.stringify(prv);
	//     args['provided-by'] = prv_str;
	// }
