 ...
 * @param {String} engine - Remote resource manager client to use (must be an instantiated {module:bbop-rest-manager} engine)
 * @returns {manager} a classic manager
 */
var manager = function(barista_location, namespace, user_token, engine){
    ...
    /**
     * Make a custom request with your own request set. This is the
     ...
     */
    anchor.start_with = function(request_set, model_id){
	// Run.
	var args = request_set.callable();	
    	anchor.apply_callbacks('prerun', [anchor]);
    	var def = anchor._engine.start(anchor._url, args);
	return def;
    };    
     ...
     * @param {module:minerva-requests#request_set} request_set
     * @param {String} [model_id] - string
     * @returns {Object|null} the response object or a null (if using an async engine)
     */
    anchor.fetch_with = function(request_set, model_id){
	// Run.
	var args = request_set.callable();	
    	anchor.apply_callbacks('prerun', [anchor]);
    	anchor._engine.fetch(anchor._url, args);
    };    
